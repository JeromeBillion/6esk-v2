...
const grant = {
  ...
  status: "active",
  subject_user_id: "dddddddd-dddd-dddd-dddd-dddddddddddd",
...
describe("privileged access alerts", () => {
  ...
  it("delivers a security webhook and appends a tenant-scoped alert outcome", async () => {
    const outcome = await sendPrivilegedAccessAlert({
      scope: { tenantId: grant.tenant_id, workspaceKey: grant.workspace_key },
      grant,
      event: "approved",
      actorUserId: "admin-1"
    });
    ...
    expect(mocks.dbQuery).toHaveBeenCalledWith(
      ...
      [grant.id, grant.tenant_id, grant.workspace_key, expect.any(String)]
    );
  });
