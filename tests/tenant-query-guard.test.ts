...
describe("tenant query guard", () => {
  ...
  it("allows tenant-scoped table queries that include tenant_key evidence", () => {
    const inspection = enforceTenantQueryGuard(
      "SELECT id FROM tickets WHERE id = $1 AND tenant_key = $2",
      { mode: "strict" }
    );
  ...
  it("blocks tenant-scoped table queries without tenant_key evidence in strict mode", () => {
    expect(() =>
  ...
  it("supports pg query config objects", () => {
    const inspection = inspectTenantQueryScope({
      text: "UPDATE users SET name = $1 WHERE id = $2 AND tenant_key = $3",
  ...
  it("allows explicit SQL suppression comments for intentional global admin reads", () => {
    const inspection = enforceTenantQueryGuard(
      "/* tenant-query-guard: ignore lead-admin inventory */ SELECT id FROM users WHERE id = $1",
      { mode: "strict" }
    );
