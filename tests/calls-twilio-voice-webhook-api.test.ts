...
describe("POST /api/calls/webhooks/twilio/voice", () => {
  ...
  it("creates an inbound call session and returns TwiML that rings desk clients", async () => {
    ...
    expect(mocks.resolveInboundCallProviderScope).toHaveBeenCalledWith({
      provider: "twilio",
      toPhone: "+16624398187",
      metadata: { accountSid: null }
    });
    expect(mocks.validateTwilioWebhookForTenant).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: expect.objectContaining({ CallSid: "CA-inbound-1" })
      })
    );
  ...
  it("rejects unresolved tenant routes before creating a call session", async () => {
