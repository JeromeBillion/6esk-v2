...
This runbook executes the `CRM_CALLS.md` staging checklist against `6esk` and Dexter-owned agent integrations.
...
# Optional downstream verification of agent event observation:
CRM_CALLS_AGENT_EVENTS_URL=https://<agent-host>/api/<events-endpoint>
...
8. Optional agent check: presence/order of `ticket.call.*` events for the same `callSessionId`.
