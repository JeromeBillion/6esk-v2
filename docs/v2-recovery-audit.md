...
- internal staff authorization now binds the role name to the internal staff tenant: `internal_admin` and `internal_support` only grant 6esk Work/impersonation authority when the user's home tenant matches `INTERNAL_STAFF_TENANT_ID`, tenant-owned roles with those names are denied, backoffice workflow owners must live in the internal staff tenant, and production env validation requires the internal staff tenant to be configured
