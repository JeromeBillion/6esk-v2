...
- Password and OAuth sign-in both route enrolled privileged users through TOTP MFA before workspace access; OAuth MFA redirects now keep the challenge token out of the URL by using a short-lived httpOnly challenge cookie consumed by the MFA endpoint, and MFA challenge verification now locks and consumes the challenge row in one transaction so concurrent replay cannot mint multiple sessions from one challenge.
- Session creation, logout, session revocation, password reset revocation, and support impersonation session mutation now include tenant-derived SQL evidence so production tenant-query guard strict mode does not block core auth flows and break-glass session updates cannot be applied by cookie hash alone; password-reset completion now locks and consumes the tenant-owned reset row inside the transaction, rechecks expiry/used state, and rejects concurrent token reuse before password mutation or audit emission.
...
- Agent outbox delivery now fails closed against the tenant's `aiAutomation` entitlement before locking queued events or dispatching Dexter runtime work; both machine delivery and admin-triggered delivery return audited `module_disabled` responses instead of running autonomous work for disabled tenants.
...
- Dexter prompt-template administration now uses the shared Admin AI access gate: list/create/activate/rollback routes require lead-admin tenant scope plus an enabled `aiAutomation` module, reject unknown top-level mutation fields, and keep audit evidence on every accepted mutation.
...
- Production env validation now requires `SECURITY_ALERT_WEBHOOK` so privileged-access and security-sensitive alert routing is configured before launch; strict production validation also rejects copied placeholder values such as `replace-with-*` before startup; real alert delivery remains deployment evidence.
...
- Transcript AI provider dispatch now follows the persisted tenant provider-secret stance in strict mode: workers retrieve tenant-scoped `managed_ai/http_secret` bridge credentials, the internal AI provider route verifies that secret against the call-session tenant before provider spend, rejects tenant/call-session mismatches, enforces the `aiAutomation` entitlement before resolving provider credentials, and bounds transcript-analysis payload size
...
- v2-native auth/session/MFA foundation on migration `0051`, including tenant security policy, session provider/device metadata, revocation evidence, TOTP enrollment/challenge flows, fail-closed role capabilities while privileged sessions are marked MFA-enrollment-required, current-session upgrade after successful MFA enrollment, tenant-admin security policy API, user session list/revoke API, password-reset session revocation, and production env validation for MFA secret encryption
...
- central API rate-limit/request-correlation middleware now backs the existing Upstash dependency: sensitive auth, admin, agent, ticket, email, WhatsApp, and outbound call routes resolve to per-minute profiles; production fails closed when Upstash credentials are absent; production env validation rejects disabled/invalid configured limits; dev/test uses an in-memory limiter; request ids are sanitized and propagated as `x-6esk-request-id`; rate-limit keys prefer Cloudflare connecting IP over spoofable forwarded headers and ignore caller-supplied tenant/workspace headers unless a future caller explicitly opts into trusted tenant-header scope after authentication or verified-secret handling
...
- tenant staff provisioning is now core-code closed for atomicity: admin user creation wraps user upsert, personal mailbox upsert, and mailbox membership persistence in one transaction, rolls back on cross-tenant conflicts or membership failures, releases the client, and only emits audit after a committed provisioning state
- 6esk Work workflow/module mutation hardening is now core-code closed: sensitive workflow case updates, timeline events, and artifact links require explicit tenant scope at the route and service boundary, while backoffice tenant-module updates reject non-catalog module keys before workspace/entitlement writes and strip stale unknown module keys from persisted module JSON on valid updates
- 6esk Work query-scope validation is now core-code closed for privileged-access reads and workflow case detail reads: invalid tenant filters return 400 at the route boundary before grant stats, case detail, timeline, or artifact services are called
- backoffice billing invoice-reference integrity is now core-code closed: manual adjustment `sourceInvoiceId` and collection-event `invoiceId` values must resolve inside the same tenant/workspace before idempotency, audit, or billing writes, zero-value adjustments are rejected at the API boundary, and migration `0072` adds composite tenant/workspace invoice reference guards for invoice lines, adjustments, and collection events
...
- provider call-site adoption for persisted tenant ingress/provider webhook secrets is now complete for WhatsApp, Resend, Twilio, Deepgram/STT, and unauthenticated machine ticket-create core code paths, including tenant-admin provider-number management; the internal Deepgram STT bridge now rejects oversized payloads before provider/tenant work and rejects oversized audio before tenant-secret lookup or Deepgram spend; remaining provider evidence is deployment/runtime validation with real credentials and dashboards
...
- WhatsApp active-template picker scope is now core-code closed: support-side template reads require ticket permissions, a session tenant, the WhatsApp module entitlement, and `tenant_id = $1` before active templates are returned to the reply composer
...
- admin bootstrap/seed tenant scope is now core-code closed for production setup: `db:seed` verifies the seed tenant, writes roles/tags/macros/SLA/users/mailboxes/memberships with explicit tenant ownership inside one transaction, blocks cross-tenant email/mailbox conflicts, migration `0073` replaces macro title global uniqueness with `(tenant_id, title)`, and the tenant-isolation gate pins these seed-script invariants
- tenant role authorization state is now core-code closed for tenant scope: migration `0074` replaces global role-name uniqueness with `(tenant_id, name)`, adds a composite user-role tenant foreign key, preflights historical cross-tenant role assignments, makes `db:seed` upsert roles by tenant/name, and pins auth/session/OAuth/backoffice owner role joins to `r.tenant_id = u.tenant_id`
- internal staff role authorization is now core-code closed for the 6esk Work boundary: `internal_admin` and `internal_support` only grant backoffice/impersonation capability when the user's home tenant matches `INTERNAL_STAFF_TENANT_ID`, tenant-owned roles with those names are denied, backoffice workflow owners must be users in the internal staff tenant, and production env validation requires the internal staff tenant to be explicit
- user auth email identity is now core-code closed for case-insensitive uniqueness: migration `0075` preflights historical `lower(email)` duplicates, normalizes stored `users.email` values, and adds `uq_users_email_lower` so password, OAuth, mailbox owner, and admin user lookup cannot select ambiguous case-variant accounts
...
- customer conversation-data route scope is now core-code closed for ticket detail/update, message detail/update, and attachment download: tenantless sessions receive `403` before SQL/object-store access, live attachment downloads cannot be bypassed through generated demo attachment IDs, demo attachment responses are allowed only for demo-mode requests, and the aggregate tenant-isolation gate covers those regressions
...
- OAuth provider mailbox sync is now core-code closed for tenant scope: Google webhook wakeups resolve only active connections joined to tenant-owned mailboxes, ambiguous provider email matches fail closed instead of choosing an arbitrary tenant, OAuth callbacks reject mailbox addresses already claimed by another tenant before creating provider-token rows, outbound OAuth send lookup requires the caller tenant, and provider sync stores inbound messages through the connected mailbox rather than message-header recipient resolution
...
| Newly closed in this consolidation | OAuth combined-token decrypt/refresh shape, production cron fail-closed behavior, Microsoft webhook static-client-state fallback, OAuth callback mailbox transaction ownership, OAuth provider mailbox sync tenant binding, explicit tenant-scoped contact module checks, tenant-scoped call review writebacks |
