...
- Dexter prompt-sandbox and output-validation foundations are now recovered in v2-native form: `buildAgentPromptSandbox` separates system constraints, tenant policy, server runtime context, untrusted event payloads, and untrusted retrieved knowledge; `validateAgentOutput` blocks unsafe generated customer-facing draft/reply output before side effects, writes tenant-scoped audit evidence, redacts prompt-safety telemetry, and records denied run-tool evidence when a run id is present
...
- The first fixture-driven AI red-team regression suite is now recovered in v2-native form: prompt safety, full-auto tool policy, generated-output validation, knowledge-ingestion safety, and prompt-sandbox trust sections are tested against direct prompt injection, indirect RAG poisoning, tool-policy bypass, secret exposure, cross-tenant/customer exfiltration, memory persistence, long-context smuggling, multilingual overrides, hostile provider output, and safe business content
- The first model/provider gateway foundation is now in place for transcript AI: tenant AI resolution returns explicit ready/disabled/misconfigured plans, bounded provider timeouts, fallback model metadata, provider-mode cost capture, and clear no-AI/invalid-configuration denial reasons without leaking provider secrets
...
Current implementation status:
- E-RAG-1 through E-RAG-4 are partially closed for launch with tenant-scoped Admin KB APIs, direct uploads, text/Markdown ingestion, publication/archive lifecycle, tenant object keys, and poison handling for unsupported binary extraction.
...
- E-RAG-5, vetted PDF/DOCX extraction, malware scanning, stronger E-RAG-8/E-RAG-11 eval coverage, Admin diagnostics polish, quotas/billing drill-down, and external connectors remain outstanding or post-launch depending on deployment scope.
...
- AI module launch blockers: 4 remaining P0 gates: bounded tool execution, tenant Knowledge Base/SOP RAG, observability/operations, and rollout/rollback. The OpenClaw review adds a concrete Dexter CRM execution track under E-OC-1 through E-OC-12, and the Knowledge Base plan adds E-RAG-1 through E-RAG-12. Bounded execution is substantially closed for the current action route, customer-service paths, WhatsApp outbox operator paths, message/attachment/resend/draft paths, inbound email/WhatsApp store paths, voice call write paths, OAuth mail provider sends/sync, call-review writebacks, the first Knowledge Base state/API/text-ingestion/publication/cited-retrieval/prompt-injection-diagnostics foundation, the first central runtime prompt-safety classifier/RAG denial layer, tenant-safe Dexter RAG context attachment, the durable run ledger/outbox timeline foundation for Dexter, the typed Dexter control-plane envelope foundation for create/cancel/wait/tool/approval/completion commands, tenant lane reservation plus lane wait/depth/stale diagnostics and stale-run/dead-letter recovery, route-level tool-call step ledger population, worker dispatch step ledger population, tenant-safe run replay diagnostics, prompt-sandbox/output-validator foundation, first fixture-driven AI red-team regression suite, first transcript-AI provider gateway foundation, and the first agent tool policy gate for prompt-safety/tool-class denial before side effects. Dexter still needs remaining policy-pipeline stages beyond the current route gate, native runtime tool-call population inside future runtime workers beyond the dispatch step, ElizaOS worker/process isolation beyond the current adapter boundary, vetted PDF/DOCX extraction, malware scanning, embeddings/vector search, broader prompt-injection eval/load drills, provider gateway rollout across all Dexter/model calls, load/release evidence, and staging rollout evidence before external AI automation.
...
- provider call-site adoption for persisted tenant ingress/provider webhook secrets beyond WhatsApp/Resend where provider verification currently reads only process env and supports tenant-specific secrets, with Twilio and Deepgram/STT remaining as the next code slices
