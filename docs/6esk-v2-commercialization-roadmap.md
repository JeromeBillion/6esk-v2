...
- provider call-site adoption for persisted tenant ingress/provider webhook secrets beyond WhatsApp/Resend/Twilio status-recording callbacks where provider verification currently reads only process env and supports tenant-specific secrets, with Twilio inbound voice/queue and Deepgram/STT remaining as the next code slices
