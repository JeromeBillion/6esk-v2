...
- Dexter run-list diagnostics now expose recent and active run evidence through `GET /api/admin/agents/[agentId]/runs`: tenant-owned integration lookup happens before run reads, status filters are limited to known run states, limits are bounded, failure summaries are redacted, and raw metadata/idempotency payloads are not returned.
