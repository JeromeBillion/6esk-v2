...
- provider call-site adoption for persisted tenant ingress/provider webhook secrets is now complete for WhatsApp, Resend, Twilio, and Deepgram/STT core code paths; tenant-admin provider-number management remains as the next configuration slice so Twilio phone/account ownership does not require manual database state
