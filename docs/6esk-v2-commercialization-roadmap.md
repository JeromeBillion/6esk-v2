...
- support tag catalog/data-model tenantization remains an active core-code blocker: route-level tenantless rejection exists, but `tags` / `ticket_tags` still need tenant-owned schema, service helpers, migration/backfill, and regression tests before tag management can be called launch-ready
