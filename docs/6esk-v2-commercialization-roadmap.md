...
- AI follow-through: lane queue observability/recovery/dead-letter hardening, remaining policy-pipeline stages beyond the current route-level tool policy gate, actual runtime step/tool-call ledger population, tenant-safe RAG runtime integration, stronger prompt-injection evals, vetted binary extraction/malware scanning, and rollout evidence while preserving native Dexter and v2 runtime files
