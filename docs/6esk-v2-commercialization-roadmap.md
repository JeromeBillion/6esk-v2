...
- Agent admin controls now fail closed when the lead-admin session lacks tenant scope before reading or mutating agent integrations, outbox metrics, outbox delivery/retry state, rollout controls, run replay evidence, run cancellation, or stale-run recovery.
