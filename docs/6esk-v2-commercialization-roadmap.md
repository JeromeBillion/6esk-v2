...
- tag catalogs, ticket-tag joins, merge-review tasks, support macros, saved views, password reset state, SLA analytics, overview analytics, voice consent history, voice operator presence, and external profile link cache state now carry tenant ownership and have regression coverage in `npm run test:tenant-isolation`
