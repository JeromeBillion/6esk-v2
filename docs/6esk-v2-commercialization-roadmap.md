...
- provider call-site adoption for persisted tenant ingress/provider webhook secrets beyond WhatsApp/Resend/Twilio callbacks where provider verification currently reads only process env and supports tenant-specific secrets, with Deepgram/STT and tenant-admin provider-number management remaining as the next code slices
