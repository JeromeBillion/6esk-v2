...
- Password and OAuth sign-in both route enrolled privileged users through TOTP MFA before workspace access; OAuth MFA redirects now keep the challenge token out of the URL by using a short-lived httpOnly challenge cookie consumed by the MFA endpoint.
