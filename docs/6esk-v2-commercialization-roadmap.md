...
- AI follow-through: remaining policy-pipeline stages beyond the current route-level tool policy gate, native/runtime worker step/tool-call ledger population beyond `/api/agent/v1/actions`, tenant-safe RAG runtime integration, stronger prompt-injection evals, vetted binary extraction/malware scanning, and rollout evidence while preserving native Dexter and v2 runtime files
