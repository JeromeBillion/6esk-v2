...
- AI follow-through: remaining policy-pipeline stages beyond the current route-level tool policy gate, native/runtime worker step/tool-call ledger population beyond `/api/agent/v1/actions`, stronger prompt-injection eval/load coverage, deployed scanner/extractor/R2 evidence, post-launch embeddings/vector search, provider-gateway rollout for future Dexter/model call sites, and rollout evidence while preserving native Dexter and v2 runtime files
