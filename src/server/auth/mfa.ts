...
type ScopedUser = Pick<SessionUser, "id" | "email" | "tenant_key" | "workspace_key" | "role_name">;
...
const MFA_SECRET_AAD_PREFIX = "auth-mfa-secret.v1";
const MFA_TOKEN_PREFIX = "auth-mfa-token.v1";
const TOTP_DIGITS = 6;
...
function resolveScope(user: Pick<ScopedUser, "id" | "tenant_key" | "workspace_key">): MfaScope {
  return {
    tenantKey: user.tenant_key?.trim() || "primary",
    workspaceKey: user.workspace_key?.trim() || "primary",
    userId: user.id
  };
}
...
function encryptionAad(scope: MfaScope) {
  return Buffer.from(`${MFA_SECRET_AAD_PREFIX}:${scope.tenantKey}:${scope.workspaceKey}:${scope.userId}`);
}
...
export function decryptMfaSecret(encrypted: string, scope: MfaScope) {
  ...
  const decipher = createDecipheriv("aes-256-gcm", readMfaEncryptionKey(), Buffer.from(ivRaw, "base64url"));
...
function validateTotpCode(secretBase32: string, code: string) {
  ...
  return buildTotp(secretBase32, "6esk").validate({ token, window: TOTP_WINDOW }) !== null;
}
...
export function isPrivilegedMfaRole(roleName: string | null | undefined) {
  return (
    ...
    roleName === SUPPORT_ADMIN_ROLE ||
    roleName === BREAK_GLASS_ROLE
  );
}

export async function isMfaRequiredForLogin(user: Pick<ScopedUser, "tenant_key" | "workspace_key" | "role_name">) {
  if (!isPrivilegedMfaRole(user.role_name)) {
    return false;
  }

  const tenantKey = user.tenant_key?.trim() || "primary";
  const workspaceKey = user.workspace_key?.trim() || "primary";
  const policy = await getTenantSecurityPolicy({ tenantKey, workspaceKey }).catch(() => null);
  if (policy) {
...
export async function listMfaFactorsForUser(user: Pick<ScopedUser, "id" | "tenant_key" | "workspace_key">) {
  ...
  const result = await db.query<MfaFactorRecord>(
    `SELECT id,
     ...
     LIMIT 25`,
    [scope.tenantKey, scope.workspaceKey, scope.userId]
  );
  return result.rows;
}

export async function hasActiveMfaFactor(user: Pick<ScopedUser, "id" | "tenant_key" | "workspace_key">) {
  const scope = resolveScope(user);
...
export async function createMfaChallenge(user: Pick<ScopedUser, "id" | "tenant_key" | "workspace_key">) {
  const scope = resolveScope(user);
  ...
  await db.query(
    `INSERT INTO auth_mfa_challenges (
     ...
     VALUES ($1, $2, $3, $4, $5)`,
    [scope.tenantKey, scope.workspaceKey, scope.userId, tokenHash(challengeToken), expiresAt]
  );
...
export async function startTotpEnrollment({
  ...
  user: Pick<ScopedUser, "id" | "email" | "tenant_key" | "workspace_key">;
  label?: string | null;
}) {
  const scope = resolveScope(user);
  ...
  const totp = buildTotp(secretBase32, factorLabel);
...
export async function verifyTotpEnrollment({
  ...
  user: Pick<ScopedUser, "id" | "tenant_key" | "workspace_key">;
  enrollmentToken: string;
  code: string;
}) {
  const scope = resolveScope(user);
  ...
  const result = await db.query<{
    ...
    `SELECT id, secret_encrypted, label
     ...
     WHERE tenant_key = $1
       ...
       AND expires_at > now()
     LIMIT 1`,
    [scope.tenantKey, scope.workspaceKey, scope.userId, hash]
  );
  ...
  await db.query(
    `UPDATE auth_mfa_enrollments
     ...
     WHERE id = $1
       AND tenant_key = $2
       AND workspace_key = $3`,
    [enrollment.id, scope.tenantKey, scope.workspaceKey]
  );
...
export async function verifyMfaChallenge({
  ...
  const scope = {
    tenantKey: challenge.tenant_key,
  ...
  for (const factor of factorResult.rows) {
    ...
    const totp = buildTotp(secretBase32, "6esk");
    const expected = totp.generate();
    const maybeValid =
      safeTokenEquals(token, expected) || totp.validate({ token, window: TOTP_WINDOW }) !== null;
    ...
    await db.query(
      `UPDATE auth_mfa_factors
       ...
       WHERE id = $1
         AND tenant_key = $2
         AND workspace_key = $3`,
      [factor.id, scope.tenantKey, scope.workspaceKey]
    );
    await db.query(
      `UPDATE auth_mfa_challenges
       ...
       WHERE id = $1
         ...
         AND workspace_key = $3
         AND used_at IS NULL`,
      [challenge.id, scope.tenantKey, scope.workspaceKey]
    );
    return {
      ok: true,
      userId: scope.userId,
  ...
  await db.query(
    `UPDATE auth_mfa_challenges
     ...
     WHERE id = $1
       ...
       AND workspace_key = $3
       AND used_at IS NULL`,
    [challengeId, scope.tenantKey, scope.workspaceKey]
  );
}
