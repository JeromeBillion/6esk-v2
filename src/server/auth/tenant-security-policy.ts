...
type TenantPolicyScope = {
  tenantId: string;
  workspaceKey?: string | null;
};

function workspaceKeyFor(scope: TenantPolicyScope) {
...
function defaultTenantSecurityPolicy(scope: TenantPolicyScope): TenantSecurityPolicy {
  return {
    tenant_id: scope.tenantId,
    workspace_key: workspaceKeyFor(scope),
...
export function isEmailAllowedByPolicy(
  email: string,
  policy: Pick<TenantSecurityPolicy, "allowed_login_domains">
) {
...
export async function getTenantSecurityPolicy(scope: TenantPolicyScope) {
...
export async function getTenantSecurityPolicyOrDefault(scope: TenantPolicyScope) {
  return (await getTenantSecurityPolicy(scope)) ?? defaultTenantSecurityPolicy(scope);
}
