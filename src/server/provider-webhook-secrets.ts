...
const SECRET_AAD = Buffer.from("tenant-provider-webhook-secret.v1", "utf8");
...
function summarizeProviderWebhookSecret(
  row: ProviderWebhookSecretRow
): ProviderWebhookSecretSummary {
  return {
    id: row.id,
  ...
  try {
    ...
    return Object.entries(parsed)
      .map(([key, value]) => {
        ...
        const [tenantKey, workspaceKey, provider, secretType, ...accountParts] = parts;
          ...
          typeof value === "string"
            ...
            : value && typeof value === "object" && !Array.isArray(value)
              ? typeof (value as Record<string, unknown>).secret === "string"
                ...
                : ""
              : "";
        if (!tenantKey || !workspaceKey || !provider || !secretType || !secret) {
          return null;
        }
        return {
          tenantKey,
...
function envSecretsFor(input: {
  ...
  return parseEnvSecrets()
    .filter((entry) => {
      const scopeMatches =
        entry.tenantKey === input.scope.tenantKey && entry.workspaceKey === input.scope.workspaceKey;
...
export async function listProviderWebhookSecrets(scope: TenantScope) {
  const result = await db.query<ProviderWebhookSecretRow>(
    `SELECT id, tenant_key, workspace_key, provider, secret_type, provider_account_id, label,
  ...
  try {
    ...
    const result = await client.query<ProviderWebhookSecretRow>(
      `INSERT INTO tenant_provider_webhook_secrets (
         tenant_key, workspace_key, provider, secret_type, provider_account_id, label, status,
       ...
       RETURNING id, tenant_key, workspace_key, provider, secret_type, provider_account_id,
...
export async function markProviderWebhookSecretUsed(secretId: string, scope: TenantScope) {
