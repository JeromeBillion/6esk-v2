...
export type AiAdminAccess = {
  user: SessionUser;
  tenantId: string;
};

function aiModuleDisabledResponse() {
...
export async function requireAiAutomationAdminAccess(): Promise<
  | { ok: true; access: AiAdminAccess }
...
> {
  ...
  if (!moduleEnabled) {
    return { ok: false, response: aiModuleDisabledResponse() };
  }
...
export async function requireAiAutomationTenantModule(tenantId: string | null | undefined) {
  ...
  if (!moduleEnabled) {
    return { ok: false as const, response: aiModuleDisabledResponse() };
  }
