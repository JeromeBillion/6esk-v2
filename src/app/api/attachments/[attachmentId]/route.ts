...
export async function GET(
  ...
  if (!isAdmin) {
    if (attachment.ticket_id) {
      const assignedUserId = await getTicketAssignment(attachment.ticket_id, tenantId);
      if (!assignedUserId || assignedUserId !== user.id) {
        return Response.json({ error: "Forbidden" }, { status: 403 });
      }
    } else {
      const allowed = await hasMailboxAccess(user.id, attachment.mailbox_id, tenantId);
      if (!allowed) {
        return Response.json({ error: "Forbidden" }, { status: 403 });
      }
    }
